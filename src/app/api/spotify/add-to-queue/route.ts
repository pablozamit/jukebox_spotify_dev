...
// ➤ Force this route to run on Node.js (not the Edge runtime)
...
// ❶ Inicializa Admin SDK usando ADC (haber corrido `gcloud auth application-default login`)
if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.applicationDefault(),
    // Asegúrate de que coincide con tu RTDB
...
export async function POST(request: Request) {
  try {
    // ❷ Leemos trackId del cuerpo de la petición
    ...
    // ❹ Si el token expiró, refrescarlo y actualizarlo en RTDB
